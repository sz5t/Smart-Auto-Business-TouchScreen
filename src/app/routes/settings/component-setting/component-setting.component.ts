...
export class ComponentSettingComponent implements OnInit {
    ...
    async ngOnInit() {
        ...
        this.attributeConfig = this.componentdic[
            "bsn-datatable"
        ].attributeConfig;
        this.fildConfig = this.componentdic["bsn-datatable"].viewCfg[0][
            "config"
        ];
        this.parameterConfig = this._editorConfig.rows[0].row.cols[0].tabs[0].viewCfg[1][
            "tabs"
        ][1].viewCfg[0]["config"];
    ...
    async getBlockConfigData(layoutId) {
        return this.apiService
            .get("common/GetComptTree", { LayoutId: layoutId })
            .toPromise();
    }
    ...
    async _changeLayoutName($event) {
        ...
        const treeDataJson = this.listToTreeData(
            TotreeBefore,
            this._funcValue[this._funcValue.length - 1]
        );

        this.treeData = treeDataJson;
        const topNode = new NzTreeNode({
            title: this._selectedModuleText,
            key: "0"
        });
        ...
        parent: [
            {
                ...
                type: "value",
                valueName: "取值参数名称",
                value: "05369c6ce6564d5eb3f044d0dfaaa689"
            }
        ],
        ajaxConfig: {
            url: "BlockSetting",
    ...
    onActivate(ev: any) {
        ...
        if (ev.node.data.type === "component") {
            if (ev.node.data.value === "bsn-datatable") {
                this._editorConfig.rows[0].row.cols[0].tabs[0].viewCfg[1][
                    "tabs"
                ][0].viewCfg = this.componentdic["bsn-datatable"].viewCfg;
                this._editorConfig.rows[0].row.cols[0].tabs[1].viewCfg[0][
                    "config"
                ] = this.componentdic["bsn-datatable"].attributeConfig;
                this._editorConfig = JSON.parse(
                    JSON.stringify(this._editorConfig)
                );
            ...
            if (ev.node.data.value === "bsn-tree") {
                this._editorConfig.rows[0].row.cols[0].tabs[0].viewCfg[1][
                    "tabs"
                ][0].viewCfg = this.componentdic["bsn-tree"].viewCfg;
                this._editorConfig.rows[0].row.cols[0].tabs[1].viewCfg[0][
                    "config"
                ...
                this.fildConfig = this.componentdic["bsn-tree"].viewCfg[0][
                    "config"
                ];
                this.parameterConfig = this._editorConfig.rows[0].row.cols[0].tabs[0].viewCfg[1][
                    "tabs"
